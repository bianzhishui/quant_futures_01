...
1. **动量家族长多腿（多头方向）三轮一致为正（独立成立）**：
   - 三个动量方案的长多腿贡献均为正（vol-TSMOM +0.20 / XSMOM +0.07 / 等权 TSMOM 亦正）；
   - → "做多上涨趋势/赢家"方向存在正贡献的**单一来源证据**。
...
4. **Carry 经稳健性诊断后不可靠（2026-09-21）**：原 🟡 超额主要来自 6 个有色品种（非有色跑输基准 +0.26 vs +0.46），且远月定义符号一致率仅 65%（有色核心品种 CU/AL/ZN ≈ 47-50% 接近随机）→ **"backwardation 溢价"作为第二来源未获印证**，倾向放弃（详见 carry_diag_plan.md）。
