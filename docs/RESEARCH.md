...
| 10 | 库存+政策维度 | [stock_macro_plan.md](stock_macro_plan.md) | 🟡 部分采纳 | 宽松期 6m +7.3% vs 收紧期 +2.0% | **政策转向（M2 宽松）增强贱极 ✅ 采纳**；**库存低位 ❌ 结构性负发现**：价格贱极事件仓单分位中位 98.7%（便宜≠库存低，两条件几乎互斥）——价格代理缺"供应收缩"要件 |
...
9. **政策转向增强（stock_macro_plan.md）**：M2 宽松期贱极事件 6m 超额 **+7.3%** vs 收紧期 **+2.0%**（3.7 倍）——信贷宽松显著增强贱极边际，采纳为筛选器增强项。
10. **结构性负发现：价格贱 ≠ 库存低**：27 个价格贱极事件仓单分位**中位 98.7%**——价格暴跌多是供给过剩（高库存）的结果，价格代理缺傅海棠"供应收缩/库存低"要件；两条件在本样本几乎互斥（这正是"低价不是买入理由"的数据注脚）。
